---
title: Build a Coding Agent Using Amp Code and Nightona
description: Step-by-step guide to building an autonomous coding agent using Amp Code CLI and Nightona sandboxes.
---
...
This guide demonstrates how to run an autonomous coding agent based on [Amp Code](https://ampcode.com/) inside a Nightona sandbox environment. The agent can develop full-stack web apps, write code in any language, install dependencies, and run scripts. It can also start and manage dev servers, and generate preview links for live apps.

---

### 1. Workflow Overview

When you launch the main module, a Nightona sandbox is created and the Amp CLI is installed inside it. The agent uses Amp's [streaming JSON mode](https://ampcode.com/manual#cli-streaming-json) for programmatic control.
...
https://8000-29baaaf7-767a-4dff-8129-1e6ec2100b3e.nightonaproxy01.net
...
The agent can also host web apps and provide you with a preview link using the [Nightona Preview Links](/docs/en/preview/) feature. When your task involves running or previewing a web application, the agent automatically reasons about this need, hosts the app, and generates a preview link for you to inspect the live result:

<Image
  src={ampSdkCodingAgentDemo}
  alt="Amp Code agent creating a Kanji flashcard app in a Nightona sandbox"
  width={600}
...
First, clone the nightona [repository](https://github.com/nightona-co/nightona.git) and navigate to the example directory:

```bash
git clone https://github.com/nightona-co/nightona.git
cd nightona/guides/typescript/amp/amp-sdk
```
...
1. Creates a new [Nightona sandbox](/docs/en/sandboxes) with the Amp API key.
2. Installs the Amp CLI globally in the sandbox.
3. Creates a PTY for streaming output from Amp.
4. Sends a Nightona-aware system prompt as the first user message (preview URL pattern + instruction to write server startup command to `/home/daytona/start.sh`).
...
A Nightona-aware system prompt is sent as the first user message. It instructs the agent to use the preview URL pattern and to write the server start command into `/home/daytona/start.sh` (instead of executing directly in Amp), then provide the preview URL:
...
The readline loop waits for user input, sends it to the agent, and displays the streamed response. If Amp produced `/home/daytona/start.sh`, the script is then launched via Nightona's session command API so long-running/background server startup does not hang Amp turns.

**Key advantages:**

- Secure, isolated execution in Nightona sandboxes
