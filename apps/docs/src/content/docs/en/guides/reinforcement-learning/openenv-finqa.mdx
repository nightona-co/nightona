---
title: Multi-Turn RL Training with OpenEnv and Nightona
description: Use parallel Nightona sandboxes with OpenEnv's FinQA environment to train Qwen3-14B on financial question answering via GRPO.
---
...
cd nightona/guides/python/reinforcement-learning/openenv
```
...
- `nightona` - Nightona SDK for sandbox management
...
Get your Nightona API key from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys) and create a `.env` file:
...
Before running any episodes, build a Nightona snapshot that pre-packages the FinQA environment server and dataset:
...
This uses Nightona's declarative Image API to clone the FinQA environment from the OpenEnv repo, install its dependencies, and pre-download the FinQA dataset from HuggingFace, all baked into a single container image for fast sandbox startup.
...
The `run.py` script demonstrates the full OpenEnv + Nightona integration in a single episode. Run it with:
...
The `NightonaProvider` from OpenEnv wraps the Nightona SDK, creating a sandbox from the pre-built snapshot and waiting for the FinQA server to become healthy:

<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```python
    from openenv.core.containers.runtime.nightona_provider import NightonaProvider

    def create_sandbox():
        provider = NightonaProvider(auto_stop_interval=0, cmd=SERVER_CMD)
...
| -------------------------- | ---------------- | ---------------------------------------------- |
| `--sandboxes`              | 500              | Number of concurrent Nightona sandboxes         |
...
| `--snapshot`               | `openenv-finqa`  | Nightona snapshot name                          |
