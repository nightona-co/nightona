---
title: Scaling Tool Execution in RL Training with veRL and Nightona
description: Run veRL's ReTool recipe on Nightona sandboxes, configure the Nightona backend, and benchmark tool-calling throughput.
---

import { Image } from 'astro:assets'

import nightonaThroughputPlot from '../../../../../assets/docs/images/verl-retool-nightona-throughput.svg'
import verlToolFigure from '../../../../../assets/docs/images/verltool-figure-2-table-2.png'

This guide demonstrates how to run [veRL's](https://github.com/verl-project/verl) ReTool recipe with Nightona sandboxes as the tool execution backend, scaling up to hundreds of tool calls per training step without hitting a concurrency ceiling.
...
### 3. Nightona as the ReTool Backend

By executing tool calls on Nightona sandboxes, the async rollout pipeline can scale to hundreds of concurrent executions without saturating the backend.
...
- **Async SDK.** The `AsyncNightona` client integrates directly with veRL's async rollout workers. Workers fire requests in parallel and process results as they arrive.
...
The chart below compares code execution throughput between Docker containers and Nightona sandboxes.

<Image
  src={nightonaThroughputPlot}
  alt="Line chart comparing throughput between Docker containers and Nightona as concurrency increases from 1 to 128."
  width={900}
  style="max-width: 100%; height: auto; margin: 1rem 0;"
/>

With Docker containers, throughput plateaus as concurrency increases. Container startup overhead dominates, and adding more parallelism doesn't help. Nightona sandboxes scale linearly and reach **98 calls/sec** at 128 concurrent — a **5.5x throughput improvement** at peak concurrency.
...
#### Export the Nightona API Key

Get your API key from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys) and export it before running the recipe or the benchmark:

```bash
export NIGHTONA_API_KEY="your_nightona_api_key"
```

### 5. Start Training

Use the existing ReTool launch script and point it at the Nightona tool config and the downloaded checkpoint:

```bash
TOOL_CFG=recipe/retool/nightona_tool_config.yaml
...
bash recipe/retool/run_qwen2-32b_dapo.sh \
  ...
  trainer.project_name=retool_nightona \
  trainer.experiment_name=qwen2.5-32b_dapo_nightona
```
...
cd nightona/guides/python/reinforcement-learning/verl-retool
...
# Nightona sandboxes (requires NIGHTONA_API_KEY and veRL)
python benchmark_tool_backends.py \
  --backend nightona \
...
Benchmarked on macOS (Docker Desktop) and Nightona cloud (includes network round-trip). Absolute numbers may vary by environment.
