...
description: Build Google ADK agents that generate and test code using Nightona's isolated sandbox environment.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide demonstrates how to use the `NightonaPlugin` for Google ADK to build an agent that generates, tests, and verifies code in a secure sandbox environment. The plugin enables agents to execute Python, JavaScript, and TypeScript code, run shell commands, and manage files within isolated Nightona sandboxes.
...
You describe the function you want in plain English, specifying the language (Python, JavaScript, or TypeScript). The agent generates the implementation, writes tests for it, and executes everything in a Nightona sandbox. If tests fail, the agent automatically fixes the code and re-runs until all tests pass. Only then does it return the verified, working code.
...
Clone the Nightona repository and navigate to the example directory:

```bash
git clone https://github.com/nightona-co/nightona
cd nightona/guides/python/google-adk/code-generator-agent/gemini
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    - `nightona-adk`: Provides the `NightonaPlugin` that enables secure code execution in Nightona sandboxes
...
1. **Nightona API key:** Get it from [Nightona Dashboard](https://app.daytona.io/dashboard/keys)
2. **Google API key:** Get it from [Google AI Studio](https://aistudio.google.com/apikey)

Create a `.env` file in your project:

```bash
NIGHTONA_API_KEY=dtn_***
GOOGLE_API_KEY=***
```
...
The `NightonaPlugin` provides tools that allow the agent to:
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    from nightona_adk import NightonaPlugin
    ...
    **What each import does:**
    ...
    - `NightonaPlugin`: Provides sandbox execution tools to the agent
    ...
    **Logging configuration:**
    ...
    - `logging.DEBUG`: Most verbose, shows all internal operations including NightonaPlugin sandbox creation and tool invocations
    ...
    With `DEBUG` level logging enabled, you can see the NightonaPlugin's internal operations, including when the sandbox is created, when the `execute_code_in_nightona` tool is invoked, and when cleanup occurs. The plugin's `plugin_name` (configurable, defaults to `nightona_plugin`) appears in these log messages, making it easy to trace plugin activity.
    :::
  </TabItem>
</Tabs>
...
### 7. Configure the Nightona Plugin
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    **Configuration options:**
    ...
    - `api_key`: Nightona API key (defaults to `NIGHTONA_API_KEY` env var)
    - `sandbox_name`: Custom name for the sandbox
    - `plugin_name`: Name displayed in logs when the plugin logs messages (defaults to `nightona_plugin`)
...
Create the agent with the Gemini model, our instruction, and the Nightona tools:
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    **Parameters explained:**
    ...
    - `tools`: List of tools from the Nightona plugin that the agent can use
  </TabItem>
</Tabs>
...
- **NightonaPlugin operations**: Sandbox creation, tool invocations (`execute_code_in_nightona`), and cleanup
- **LLM requests and responses**: The prompts sent to Gemini and the responses received
- **Plugin registration**: Confirmation that the `nightona_plugin` was registered with the agent
...
```
DEBUG:nightona_adk.plugin:Nightona sandbox created: e38f8574-48ac-48f1-a0ff-d922d02b0fcb
INFO:google_adk.google.adk.plugins.plugin_manager:Plugin 'nightona_plugin' registered.
```

The NightonaPlugin creates an isolated sandbox and registers itself with the agent.
...
The agent writes both the implementation and test cases, then calls the `execute_code_in_nightona` tool:
...
-----------------------------------------------------------
Function calls:
name: execute_code_in_nightona, args: {'code': "...", 'language': 'typescript'}
```

**Step 4: Code execution in sandbox**

```
DEBUG:nightona_adk.plugin:Before tool: execute_code_in_nightona
DEBUG:nightona_adk.tools:Executing typescript code (length: 1570 chars)
DEBUG:nightona_adk.tools:Code execution completed with exit_code: 0
DEBUG:nightona_adk.plugin:After tool: execute_code_in_nightona
```
...
```
INFO:nightona_adk.plugin:Deleting Nightona sandbox...
INFO:nightona_adk.plugin:Nightona sandbox deleted.
...
- **Secure execution:** Code runs in isolated Nightona sandboxes, not on your machine
...
For the complete API reference of the Nightona ADK plugin, including all available tools and configuration options, see the [nightona-adk documentation](https://github.com/nightona-co/nightona-adk-plugin#available-tools).
