...
description: Serve OpenAI's gpt-oss reasoning model on a Nightona GPU sandbox with SGLang, and put it to work classifying documents with guaranteed-structure output.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide demonstrates how to serve large language models on a Nightona [GPU sandbox](/docs/en/sandboxes#gpu-sandboxes) with [SGLang](https://docs.sglang.ai/), and query them from anywhere through a token-authenticated preview URL. The worked example serves [gpt-oss-20b](https://huggingface.co/openai/gpt-oss-20b), OpenAI's open-weights reasoning model, but the same script serves any model SGLang supports.
...
Clone the [Nightona repository](https://github.com/nightona-co/nightona.git), navigate to the example directory, and install into a virtual environment:

```bash
git clone https://github.com/nightona-co/nightona.git
cd nightona/guides/python/model-serving/sglang
python3 -m venv venv
...
Paste the printed `export` lines into your shell, then use any OpenAI-compatible client. The only Nightona-specific detail is the `x-nightona-preview-token` header; everything else is the standard OpenAI API surface, including `stream=True` for token streaming.
...
In this example `get_weather` runs in the same process as the client. Nightona makes a stronger pattern natural: give each chat session its own CPU sandbox and execute the model's tool calls there with `sandbox.process.code_run(...)`, so model-written code runs isolated from your machine and from other sessions. The GPU sandbox where the model thinks, and CPU sandboxes where its decisions execute, both on Nightona.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```python
    context = (
        "The Nightona platform provides isolated sandboxes for AI agents to safely execute code. " * 60
    )
    for attempt in (1, 2):
...
Everything so far used the default preview auth: the token in the `x-nightona-preview-token` header, the best fit for code you control. The alternatives, in increasing order of openness:
