---
title: Run OpenCode with the Nightona Plugin
description: Use the Nightona OpenCode plugin to run each OpenCode session in a Nightona sandbox, with automatic git sync to local branches.
---
...
This guide demonstrates how to run the [Nightona OpenCode plugin](https://www.npmjs.com/package/@nightona/opencode) which integrates Nightona sandboxes and OpenCode. When the plugin is active, all agent operations occur in secure sandboxes, with one sandbox per OpenCode session. The plugin also has the ability to sync changes between sandboxes and local Git branches.

### 1. Workflow Overview

When you run OpenCode with the Nightona plugin, sandboxes are created automatically inside OpenCode sessions. Operations such as running code, installing dependencies, and starting servers occur in the sandbox.

<Image
  src={opencodePluginGif}
  alt="OpenCode with Nightona plugin — sandbox toast and git branches"
  width={600}
...
Add the Nightona plugin to your project by creating or editing `opencode.json` in the project directory:
...
The Nightona plugin consists of several modules that provide custom tools, hooks for events, connections from sessions to sandboxes and system prompt transforms. These modules ensure every agent action runs in a [Nightona sandbox](/docs/en/sandboxes) and that changes sync to local Git branches:
...
The custom tools module registers overrides for OpenCode's built-in tools so that every file and process operation goes through the Nightona SDK. It receives the project `id` and `worktree` from the plugin context and returns a tool map:
...
For example, the plugin implementation of the bash tool uses the Nightona SDK to run the command in the sandbox:
...
export async function systemPromptTransform(ctx: PluginInput, repoPath: string) {
  return async (input: ExperimentalChatSystemTransformInput, output: ExperimentalChatSystemTransformOutput) => {
    output.system.push(
      [
        '## Nightona Sandbox Integration',
        'This session is integrated with a Nightona sandbox.',
...
```typescript
export async function eventHandlers(ctx: PluginInput, sessionManager: NightonaSessionManager, repoPath: string) {
  const projectId = ctx.project.id
  const worktree = ctx.project.worktree
...
For more information on how the sync is implemented, see the [SessionGitManager](https://github.com/nightona-co/nightona/blob/main/libs/opencode-plugin/.opencode/plugin/nightona/git/session-git-manager.ts) class.
...
- **macOS:** `~/.local/share/opencode/storage/nightona/[projectid].json`
...
- Secure, isolated execution: each OpenCode session runs in its own Nightona sandbox
