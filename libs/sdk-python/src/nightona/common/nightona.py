...
class CodeLanguage(str, Enum):
    """Programming languages supported by Nightona
...
class NightonaConfig(BaseModel):
    """Configuration options for initializing the Nightona client.

    Attributes:
        api_key (str | None): API key for authentication with the Nightona API. If not set, it must be provided
            via the environment variable `NIGHTONA_API_KEY`, or a JWT token must be provided instead.
        jwt_token (str | None): JWT token for authentication with the Nightona API. If not set, it must be provided
        ...
        target (str | None): Target runner location for the Sandbox. Default region for the organization is used
            if not set here or in the environment variable `NIGHTONA_TARGET`.
        ...
        otel_enabled (bool | None): Enable OpenTelemetry tracing for SDK operations. Defaults
            to `None`, which falls back to the `NIGHTONA_OTEL_ENABLED` environment variable.
    ...
    Example:
        ...
        ```
        ```python
        config = NightonaConfig(jwt_token="your-jwt-token", organization_id="your-organization-id")
        ```
    """
