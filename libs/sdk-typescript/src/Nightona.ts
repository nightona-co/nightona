...
import { NightonaEnvReader, RUNTIME, Runtime } from './utils/Runtime'
...
/**
 * Configuration options for initializing the Nightona client.
 *
 * @interface
 * @property {string} apiKey - API key for authentication with the Nightona API
 * @property {string} jwtToken - JWT token for authentication with the Nightona API. If not set, it must be provided
 ...
 *     target: "us"
 * };
 * const nightona = new Nightona(config);
 */
export interface NightonaConfig {
 ...
 * Provides methods for creating, managing, and interacting with Nightona Sandboxes.
 * Can be initialized either with explicit configuration or using environment variables.
 *
 * @property {VolumeService} volume - Service for managing Nightona Volumes
 * @property {SnapshotService} snapshot - Service for managing Nightona Snapshots
 *
 * @example
 * // Using environment variables
 * // Uses NIGHTONA_API_KEY, NIGHTONA_API_URL, NIGHTONA_TARGET
 ...
 *
 * @example
 * // Disposes nightona and flushes traces when done
 * await using nightona = new Nightona({
 *   otelEnabled: true,
 * });
 * @class
 */
export class Nightona implements AsyncDisposable {
  ...
  /**
   * Creates a new Nightona client instance.
   *
   * @param {NightonaConfig} [config] - Configuration options
   * @throws {NightonaAuthenticationError} When no credentials are provided (neither API key nor JWT token)
   * @throws {NightonaAuthenticationError} When JWT token is provided without an organization ID
   */
  constructor(config?: NightonaConfig) {
    ...
    let _envReader: NightonaEnvReader | null | undefined
    const envReader = (): NightonaEnvReader | null => {
      if (_envReader === undefined) {
        _envReader = RUNTIME !== Runtime.BROWSER ? new NightonaEnvReader() : null
      }
      return _envReader
    }
    ...
    if (
      ...
      if (reader) {
        this.apiKey = this.apiKey || (this.jwtToken ? undefined : reader.get('NIGHTONA_API_KEY'))
        this.jwtToken = this.jwtToken || reader.get('NIGHTONA_JWT_TOKEN')
        this.organizationId = this.organizationId || reader.get('NIGHTONA_ORGANIZATION_ID')
        apiUrl = apiUrl || reader.get('NIGHTONA_API_URL') || reader.get('NIGHTONA_SERVER_URL')
        this.target = this.target || reader.get('NIGHTONA_TARGET')

        if (reader.get('NIGHTONA_SERVER_URL') && !reader.get('NIGHTONA_API_URL')) {
          console.warn(
            '[Deprecation Warning] Environment variable `NIGHTONA_SERVER_URL` is deprecated and will be removed in future versions. Use `NIGHTONA_API_URL` instead.',
          )
        }
      }
    }
    ...
    if (!this.apiKey && !this.jwtToken) {
      throw new NightonaAuthenticationError(
        'Authentication credentials not found. Set NIGHTONA_API_KEY, or both NIGHTONA_JWT_TOKEN and NIGHTONA_ORGANIZATION_ID.' +
    ...
    const configuration = new Configuration({
      ...
      baseOptions: {
        headers: {
          ...
          'X-Nightona-Source': sdkLabel,
          'X-Nightona-SDK-Version': packageJson.version,
    ...
    const axiosInstance = Nightona.createAxiosInstance()
    ...
    const otelEnabled =
      ...
      env?.get('NIGHTONA_OTEL_ENABLED') === 'true' ||
      env?.get('NIGHTONA_EXPERIMENTAL_OTEL_ENABLED') === 'true'
    ...
    this.otelSdk = new NodeSDK({
      resource: resourceFromAttributes({
        ...
        [ATTR_SERVICE_NAME]: 'nightona-typescript-sdk',
      }),
      instrumentations: [
   ...
   * };
   * const sandbox = await nightona.create(params, { timeout: 100 });
   */
  public async create(params?: CreateSandboxFromSnapshotParams, options?: { timeout?: number }): Promise<Sandbox>
  /**
   * Creates Sandboxes from specified image available on some registry or declarative Nightona Image. You can specify various parameters,
   * including resources, language, image, environment variables, and volumes. Nightona creates snapshot from
   ...
   *
   * @example
   * const sandbox = await nightona.create({ image: 'debian:12.9' }, { timeout: 90, onSnapshotCreateLogs: console.log });
   *
   * @example
   * // Create a custom sandbox
   ...
   *     autoDeleteInterval: 120
   * };
   * const sandbox = await nightona.create(params, { timeout: 100, onSnapshotCreateLogs: console.log });
   */
  public async create(
  ...
  public async create(
    ...
    if (options.timeout < 0) {
      throw new NightonaValidationError('Timeout must be a non-negative number')
    }
    ...
    if (
      ...
      throw new NightonaValidationError('autoStopInterval must be a non-negative integer')
    }
    ...
    if (
      ...
      throw new NightonaValidationError('autoArchiveInterval must be a non-negative integer')
    }
    ...
    try {
        ...
        while (sandboxInstance.state === SandboxState.PENDING_BUILD) {
          if (options.timeout) {
            ...
            if (elapsed > options.timeout) {
              throw new NightonaTimeoutError(
      ...
      if (error instanceof NightonaTimeoutError) {
        ...
        throw new NightonaTimeoutError(errMsg, error.statusCode, error.headers, error.errorCode)
      }
   ...
   *
   * @example
   * const sandbox = await nightona.get('my-sandbox-id-or-name');
   ...
   *
   * @example
   * for await (const sandbox of nightona.list({ labels: { env: 'dev' } })) {
    ...
    async function* generator(): AsyncGenerator<Sandbox> {
      ...
      while (true) {
        ...
        for (const sandbox of response.data.items) {
          ...
          yield new Sandbox(sandbox, structuredClone(clientConfig), Nightona.createAxiosInstance(), sandboxApi)
        }
   ...
   *
   * @example
   * const sandbox = await nightona.get('my-sandbox-id');
   * // Wait up to 60 seconds for the sandbox to start
   * await nightona.start(sandbox, 60);
   */
  @WithInstrumentation()
   ...
   * @example
   * const sandbox = await nightona.get('my-sandbox-id');
   * const forked = await nightona._experimental_fork(sandbox, { name: 'my-fork' });
    ...
    axiosInstance.interceptors.response.use(
      ...
      (error) => {
        if (error instanceof AxiosError) {
          throw createAxiosNightonaError(error)
        }

        throw new NightonaError(error instanceof Error ? error.message : String(error))
      },
    )
