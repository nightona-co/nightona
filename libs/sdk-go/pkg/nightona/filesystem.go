...
func (f *FileSystemService) DownloadFile(ctx context.Context, remotePath string, localPath *string) ([]byte, error) {
	return withInstrumentation(ctx, f.otel, "FileSystem", "DownloadFile", func(ctx context.Context) ([]byte, error) {
		...
		if err != nil {
			return nil, errors.NewNightonaError("Failed to read file", 0, nil)
		}

		if localPath != nil {
			if err := os.WriteFile(*localPath, data, 0644); err != nil {
				return nil, errors.NewNightonaError(fmt.Sprintf("Failed to write file: %v", err), 0, nil)
			}
		}
...
func (f *FileSystemService) UploadFileStream(ctx context.Context, source io.Reader, remotePath string, opts ...UploadStreamOption) error {
	if source == nil {
		return errors.NewNightonaError("source must not be nil", 0, nil)
	}
...
func (f *FileSystemService) UploadFile(ctx context.Context, source any, destination string) error {
	return withInstrumentationVoid(ctx, f.otel, "FileSystem", "UploadFile", func(ctx context.Context) error {
		...
		case string:
			...
			if err != nil {
				return errors.NewNightonaError(fmt.Sprintf("Failed to read file: %v", err), 0, nil)
			}
		default:
			return errors.NewNightonaError("Invalid source type", 0, nil)
		}

		// Create a temporary file for the toolbox API
		tmpFile, err := os.CreateTemp("", "nightona-upload-*")
		if err != nil {
			return errors.NewNightonaError(fmt.Sprintf("Failed to create temp file: %v", err), 0, nil)
		}
		defer os.Remove(tmpFile.Name())
		defer tmpFile.Close()

		if _, err := tmpFile.Write(data); err != nil {
			return errors.NewNightonaError(fmt.Sprintf("Failed to write temp file: %v", err), 0, nil)
		}

		// Seek to the beginning for reading
		if _, err := tmpFile.Seek(0, 0); err != nil {
			return errors.NewNightonaError(fmt.Sprintf("Failed to seek temp file: %v", err), 0, nil)
		}
