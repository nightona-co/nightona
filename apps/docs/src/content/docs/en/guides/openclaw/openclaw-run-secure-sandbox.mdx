---
title: Run OpenClaw in a Nightona Sandbox via CLI
description: Learn how to run OpenClaw inside a Nightona sandbox using the Nightona CLI for secure, isolated, and always-on AI assistant with Telegram and WhatsApp.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide walks you through setting up [OpenClaw](https://openclaw.ai/) inside a Nightona sandbox and configuring Telegram and WhatsApp channels.

Running OpenClaw in a Nightona sandbox keeps your AI assistant isolated from your local machine, provides a secure environment for code execution, and ensures your bot stays online 24/7 without tying up your personal computer.
...
```bash
nightona sandbox create --name openclaw --snapshot nightona-medium --auto-stop 0
```

OpenClaw comes preinstalled in the default Nightona snapshot, so the command above is all you need.

:::note
The `--auto-stop 0` flag disables automatic shutdown, keeping OpenClaw accessible until you manually stop or delete the sandbox. The `nightona-medium` snapshot is required because the OpenClaw gateway needs a minimum of 2GB memory.
:::
...
```bash
nightona preview-url openclaw --port 18789
```
