...
// short-circuit NightonaEnvReader to read process.env only — never the
...
jest.mock('../utils/Runtime', () => {
  ...
  return { ...actual, NightonaEnvReader: TestEnvReader }
})
...
describe('Nightona', () => {
  beforeEach(() => {
    ...
    delete process.env.NIGHTONA_API_KEY
    delete process.env.NIGHTONA_JWT_TOKEN
    delete process.env.NIGHTONA_ORGANIZATION_ID
    delete process.env.NIGHTONA_API_URL
    delete process.env.NIGHTONA_SERVER_URL
    delete process.env.NIGHTONA_TARGET
  ...
  it('uses explicit api key config and builds dependent services', async () => {
    const { Nightona } = await import('../Nightona')

    const instance = new Nightona({
      apiKey: 'api-key',
      apiUrl: 'https://example.nightona.test/api',
      target: 'eu',
    })
  ...
  it('reads constructor values from env when config omitted', async () => {
    process.env.NIGHTONA_API_KEY = 'env-key'
    process.env.NIGHTONA_API_URL = 'https://env.nightona/api'
    process.env.NIGHTONA_TARGET = 'us'
    ...
    expect(firstConfigArg.basePath).toBe('https://env.nightona/api')
  ...
  it('supports deprecated serverUrl config', async () => {
    ...
    new Nightona({ apiKey: 'k', serverUrl: 'https://legacy.nightona/api', target: 'us' })

    const firstConfigArg = mockConfigurationCtor.mock.calls[0][0] as {
      basePath: string
    }

    expect(firstConfigArg.basePath).toBe('https://legacy.nightona/api')
  })

  it('reads deprecated server url from env and warns once', async () => {
    process.env.NIGHTONA_API_KEY = 'env-key'
    process.env.NIGHTONA_SERVER_URL = 'https://server.nightona/api'
    process.env.NIGHTONA_TARGET = 'us'
    ...
    expect(firstConfigArg.basePath).toBe('https://server.nightona/api')
  ...
  it('supports jwt auth with organization header', async () => {
    ...
    expect(firstConfigArg.baseOptions.headers['X-Nightona-Organization-ID']).toBe('org-1')
  })
  ...
  it('throws when no credentials are provided', async () => {
    ...
    delete process.env.NIGHTONA_JWT_TOKEN
    delete process.env.NIGHTONA_ORGANIZATION_ID
    expect(() => new Nightona()).toThrow('Authentication credentials not found.')
  })
  ...
  it('throws when jwt auth has no organization id', async () => {
    ...
    ).toThrow('NIGHTONA_ORGANIZATION_ID is required when authenticating with NIGHTONA_JWT_TOKEN.')
  })

  it('reads jwt credentials from env when config omits them', async () => {
    process.env.NIGHTONA_JWT_TOKEN = 'env-jwt'
    process.env.NIGHTONA_ORGANIZATION_ID = 'env-org'
    process.env.NIGHTONA_API_URL = 'https://env-jwt.nightona/api'
    process.env.NIGHTONA_TARGET = 'eu'
    ...
    expect(firstConfigArg.baseOptions.headers['X-Nightona-Organization-ID']).toBe('env-org')
  })

  it('throws unsupported language in create', async () => {
    const { Nightona } = await import('../Nightona')
    const instance = new Nightona({ apiKey: 'k', apiUrl: 'http://api', target: 'us' })
  ...
  it('wraps NightonaTimeoutError from sandbox startup in create', async () => {
    const { Nightona } = await import('../Nightona')
    const { Sandbox } = await import('../Sandbox')
    const { NightonaTimeoutError } = await import('../errors/NightonaError')
    const instance = new Nightona({ apiKey: 'k', apiUrl: 'http://api', target: 'us' })
    ...
    ;(Sandbox as jest.Mock).mockImplementationOnce((dto: { id: string; state?: string }) => ({
      ...
      stop: jest.fn(),
      delete: jest.fn(),
      waitUntilStarted: jest.fn().mockRejectedValue(new NightonaTimeoutError('slow start')),
      _experimental_fork: jest.fn(),
    }))
