...
type Sandbox struct {
	...
	Snapshot       *string                // Nightona snapshot used to create the sandbox
...
func (s *Sandbox) doStartWithTimeout(ctx context.Context, timeout time.Duration) error {
	if timeout < 0 {
		return errors.NewNightonaError("Timeout must be a non-negative number", 0, nil)
	}
...
func (s *Sandbox) doWaitForStart(ctx context.Context, timeout time.Duration) error {
	...
	for {
		select {
		case <-ctx.Done():
			return errors.NewNightonaTimeoutError(fmt.Sprintf("Sandbox did not start within %s", timeout))
		case <-timer.C:
			...
			if s.State == apiclient.SANDBOXSTATE_ERROR || s.State == apiclient.SANDBOXSTATE_BUILD_FAILED {
				return errors.NewNightonaError("Sandbox failed to start", 0, nil)
			}
...
func (s *Sandbox) doWaitForStop(ctx context.Context, timeout time.Duration) error {
	...
	for {
		select {
		case <-ctx.Done():
			return errors.NewNightonaTimeoutError(fmt.Sprintf("Sandbox did not stop within %s", timeout))
		case <-timer.C:
			if err := s.RefreshData(ctx); err != nil {
				return err
			}
...
// "x-nightona-preview-token" request header.
//
// Example:
//
...
func (s *Sandbox) doSetAutoArchiveInterval(ctx context.Context, intervalMinutes *int) error {
	if intervalMinutes == nil {
		return errors.NewNightonaError("intervalMinutes cannot be nil", 0, nil)
	}
...
func (s *Sandbox) doExperimentalForkWithTimeout(ctx context.Context, name *string, timeout time.Duration) (*Sandbox, error) {
	if timeout < 0 {
		return nil, errors.NewNightonaError("Timeout must be a non-negative number", 0, nil)
	}
...
func (s *Sandbox) waitForSnapshotComplete(ctx context.Context) error {
	...
	for s.State == apiclient.SANDBOXSTATE_SNAPSHOTTING {
		...
		if s.State == apiclient.SANDBOXSTATE_ERROR || s.State == apiclient.SANDBOXSTATE_BUILD_FAILED {
			return errors.NewNightonaError(
		...
		select {
		case <-ctx.Done():
			return errors.NewNightonaError("Sandbox snapshot did not complete within the timeout period", 0, nil)
...
func (s *Sandbox) waitForPauseComplete(ctx context.Context) error {
	...
	for s.State == apiclient.SANDBOXSTATE_PAUSING {
		...
		select {
		case <-ctx.Done():
			return errors.NewNightonaError("Sandbox pause did not complete within the timeout period", 0, nil)
		case <-time.After(checkInterval):
		}
...
func (s *Sandbox) doResizeWithTimeout(ctx context.Context, resources *types.Resources, timeout time.Duration) error {
	if resources == nil {
		return errors.NewNightonaError("Resources must not be nil", 0, nil)
	}
...
func (s *Sandbox) WaitForResize(ctx context.Context, timeout time.Duration) error {
	return withInstrumentationVoid(ctx, s.otel, "Sandbox", "WaitForResize", func(ctx context.Context) error {
		...
		for {
			select {
			case <-ctx.Done():
				return errors.NewNightonaTimeoutError(fmt.Sprintf("Sandbox resize did not complete within %s", timeout))
			case <-timer.C:
				if err := s.RefreshData(ctx); err != nil {
					return err
				}
				...
				if s.State == apiclient.SANDBOXSTATE_ERROR || s.State == apiclient.SANDBOXSTATE_BUILD_FAILED {
					return errors.NewNightonaError("Sandbox resize failed", 0, nil)
				}
