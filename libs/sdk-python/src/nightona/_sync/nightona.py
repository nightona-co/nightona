...
from nightona_api_client import ApiClient, ConfigApi, Configuration, CreateBuildInfo, CreateSandbox
from nightona_api_client import GpuType as SyncGpuType
from nightona_api_client import ObjectStorageApi, SandboxApi, SandboxState, SandboxVolume, SnapshotsApi
from nightona_api_client import VolumesApi as VolumesApi
...
class Nightona:
    ...
    This class provides methods to create, manage, and interact with Nightona Sandboxes.
    ...
    Example:
        ...
        ```python
        nightona = Nightona()  # Uses NIGHTONA_API_KEY, NIGHTONA_API_URL
        ...
        async with Nightona(config) as nightona:
            sandbox = nightona.create()
    ...
    def __init__(self, config: NightonaConfig | None = None):
        ...
        self._api_client.default_headers["X-Nightona-Source"] = "sdk-python"
        ...
        # pool is closed when the last reference (this Nightona or any Sandbox it
        # produced) goes away.
        ...
        env = env_reader or NightonaEnvReader()
        otel_enabled = (
            (config and config.otel_enabled)
            or (config and config._experimental and config._experimental.get("otelEnabled"))
            or env.get("NIGHTONA_OTEL_ENABLED") == "true"
            or env.get("NIGHTONA_EXPERIMENTAL_OTEL_ENABLED") == "true"
        )
        if otel_enabled:
    ...
    def create(
        ...
        *,
        ...
        Example:
            ...
            sandbox = nightona.create(params, timeout=40)
            ```
        """
    ...
    def create(
        ...
        *,
        ...
        Example:
            ...
            ```python
            sandbox = nightona.create(CreateSandboxFromImageParams(image="debian:12.9"))
            ```
    ...
    def delete(self, sandbox: Sandbox, timeout: float = 60) -> None:
        ...
        Example:
            ...
            sandbox = nightona.create()
            # ... use sandbox ...
            nightona.delete(sandbox)  # Clean up when done
            ```
        """
    ...
    def get(self, sandbox_id_or_name: str) -> Sandbox:
        ...
        Example:
            ```python
            sandbox = nightona.get("my-sandbox-id-or-name")
            print(sandbox.state)
            ```
        """
    ...
    def list(
        ...
        Example:
            ...
            for sandbox in nightona.list(ListSandboxesQuery(labels={"env": "dev"})):
                print(sandbox.id)
            ```
        """
    ...
    @with_instrumentation(name="Nightona.list.fetch_page")
