---
title: Build a Plan-and-Execute Data Agent With LangGraph and Nightona
description: Wire a six-node LangGraph state machine that runs an end-to-end ETL plus analytical-SQL workflow in an isolated Nightona sandbox, with structured planning and per-step retry recovery.
---
...
You ask the agent a natural-language data question. The planner emits an ordered list of atomic plan steps as structured output. For each step the executor generates Python code and runs it in a Nightona sandbox. A deterministic `check` node advances to the next step on success, retries the current step (with the failing code as context, up to `max_attempts`) on failure, or routes to the summarizer once the plan is complete. The summarizer produces a final natural-language answer and a `cleanup` node deletes the sandbox.
...
cd nightona/guides/python/langgraph/plan-and-execute-data-agent
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```bash
    pip install -U langgraph langchain-core langchain-anthropic nightona pydantic python-dotenv
    ```
    ...
    - `nightona`: Nightona Python SDK for sandbox provisioning and code execution
...
Before walking through the implementation, here are the key concepts the code relies on: the choice of agent pattern (plan-and-execute vs. ReAct), LangGraph's `StateGraph` with task-specific state, how state updates flow through nodes, structured-output planning, and the Nightona code interpreter.
...
a single chunk of Python code that the executor stage will then write and run in a persistent Nightona sandbox.
...
ONLY Python code that accomplishes the current step. The code runs in a persistent Nightona sandbox; prior
...
```text
[provision] creating Nightona sandbox...
...
- **Persistent interpreter context:** Nightona's `code_interpreter.run_code` shares one Python interpreter across all `execute` calls, so imports, variables, and functions defined in earlier steps stay in scope, exactly the behavior the LLM's plan assumes.
- **Secure sandbox execution:** Every line of LLM-generated Python runs in an isolated Nightona sandbox, not on your machine.
