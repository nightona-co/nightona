...
// Package nightona provides a Go SDK for interacting with the Nightona platform.
//
// The Nightona SDK enables developers to programmatically create, manage, and interact
...
//
//	client, err := nightona.NewClientWithConfig(&types.NightonaConfig{
//	    APIKey: "your-api-key",
...
// Client is the main entry point for interacting with the Nightona platform.
//
...
type Client struct {
	...
	apiURL          string             // Base URL for the Nightona API
...
// NewClient creates a new Nightona client with default configuration.
//
// NewClient reads configuration from environment variables:
//   - NIGHTONA_API_KEY or NIGHTONA_JWT_TOKEN for authentication (one is required)
//   - NIGHTONA_ORGANIZATION_ID (required when using JWT token)
//   - NIGHTONA_API_URL for custom API endpoint
//   - NIGHTONA_TARGET for target environment
//
...
// NewClientWithConfig creates a new Nightona client with a custom configuration.
//
...
func NewClientWithConfig(config *types.NightonaConfig) (*Client, error) {
	client := &Client{
	...
	if client.apiKey == "" && client.jwtToken == "" {
		return nil, errors.NewNightonaError("Authentication credentials not found. Set NIGHTONA_API_KEY, or both NIGHTONA_JWT_TOKEN and NIGHTONA_ORGANIZATION_ID. These can also be provided via NightonaConfig.", 0, nil)
	}
	...
	if client.jwtToken != "" && client.organizationID == "" {
		return nil, errors.NewNightonaError("NIGHTONA_ORGANIZATION_ID is required when authenticating with NIGHTONA_JWT_TOKEN. It can also be provided via NightonaConfig.", 0, nil)
	}
	...
	apiCfg.AddDefaultHeader("X-Nightona-Source", sdkSource)
	apiCfg.AddDefaultHeader("X-Nightona-SDK-Version", Version)
	...
	if client.jwtToken != "" {
		apiCfg.AddDefaultHeader("X-Nightona-Organization-ID", client.organizationID)
	}
	...
	if otelEnabled {
		...
		if err != nil {
			return nil, errors.NewNightonaError(fmt.Sprintf("failed to initialize OpenTelemetry: %v", err), 0, nil)
		}
...
// handleAPIError converts API client errors to Nightona error types
func (c *Client) handleAPIError(err error, httpResp *http.Response) error {
	if httpResp == nil {
		return errors.NewNightonaError(err.Error(), 0, nil)
	}
	...
	case http.StatusNotFound:
		return errors.NewNightonaNotFoundError(message, httpResp.Header)
	case http.StatusTooManyRequests:
		return errors.NewNightonaRateLimitError(message, httpResp.Header)
	default:
		return errors.NewNightonaError(message, httpResp.StatusCode, httpResp.Header)
	}
}
...
func (c *Client) createToolboxClient(proxyURL string, sandboxID string) (*toolbox.APIClient, error) {
	...
	cfg.AddDefaultHeader("X-Nightona-Source", sdkSource)
	cfg.AddDefaultHeader("X-Nightona-SDK-Version", Version)
	cfg.UserAgent = "sdk-go/" + Version

	if c.jwtToken != "" {
		cfg.AddDefaultHeader("X-Nightona-Organization-ID", c.organizationID)
	}
...
func (c *Client) doCreate(ctx context.Context, params any, opts ...func(*options.CreateSandbox)) (*Sandbox, error) {
	...
	default:
		return nil, errors.NewNightonaError(fmt.Sprintf("invalid %s: %s. Supported languages: python, javascript, typescript", types.CodeToolboxLanguageLabel, baseParams.Language), 0, nil)
	}
	...
	if baseParams.AutoStopInterval != nil && *baseParams.AutoStopInterval < 0 {
		return nil, errors.NewNightonaError("autoStopInterval must be a non-negative integer", 0, nil)
	}
	if baseParams.AutoArchiveInterval != nil && *baseParams.AutoArchiveInterval < 0 {
		return nil, errors.NewNightonaError("autoArchiveInterval must be a non-negative integer", 0, nil)
	}
	...
	if sandboxResp.GetState() == apiclient.SANDBOXSTATE_ERROR || sandboxResp.GetState() == apiclient.SANDBOXSTATE_BUILD_FAILED {
		return nil, errors.NewNightonaError("Sandbox failed to start", 0, nil)
	}
...
// a [errors.NightonaNotFoundError] is returned.
//
// Example:
//
//	sandbox, err := client.Get(ctx, "my-sandbox")
//	if err != nil {
//	    var notFound *errors.NightonaNotFoundError
...
func (c *Client) doGet(ctx context.Context, sandboxIDOrName string) (*Sandbox, error) {
	if sandboxIDOrName == "" {
		return nil, errors.NewNightonaError("sandbox ID or name is required", 0, nil)
	}
...
func (c *Client) streamBuildLogsToChannel(ctx context.Context, sandboxID string, logChan chan<- string) error {
	...
	if c.jwtToken != "" && c.organizationID != "" {
		httpReq.Header.Set("X-Nightona-Organization-ID", c.organizationID)
	}
