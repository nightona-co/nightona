...
func TestNewClient(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				"NIGHTONA_JWT_TOKEN":       "test-jwt-token",
				"NIGHTONA_ORGANIZATION_ID": "test-org-id",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
		...
		},
		{
			...
			envVars: map[string]string{
				"NIGHTONA_API_KEY": "test-api-key",
				"NIGHTONA_API_URL": "https://custom.api.url/api",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
		...
		},
		{
			...
			envVars:       map[string]string{},
			expectedError: true,
			errorContains: "Authentication credentials not found. Set NIGHTONA_API_KEY, or both NIGHTONA_JWT_TOKEN and NIGHTONA_ORGANIZATION_ID",
		},
		{
			...
			expectedError: true,
			errorContains: "NIGHTONA_ORGANIZATION_ID is required when authenticating with NIGHTONA_JWT_TOKEN",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("NIGHTONA_API_KEY", "")
			t.Setenv("NIGHTONA_API_URL", "")
			t.Setenv("NIGHTONA_JWT_TOKEN", "")
			t.Setenv("NIGHTONA_ORGANIZATION_ID", "")
...
func TestNewClientWithConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				"NIGHTONA_API_KEY": "env-api-key",
				"NIGHTONA_API_URL": "https://env.api.url/api",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
				assert.Equal(t, "config-api-key", c.apiKey)
...
func TestHandleAPIError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedType:   "*errors.NightonaNotFoundError",
		...
		},
		{
			...
			expectedType:   "*errors.NightonaRateLimitError",
		...
		},
		{
			...
			expectedType:   "*errors.NightonaError",
...
func TestGet(t *testing.T) {
	t.Run("error when sandbox ID is empty", func(t *testing.T) {
		t.Setenv("NIGHTONA_API_KEY", "test-api-key")
		t.Setenv("NIGHTONA_API_URL", "")
		t.Setenv("NIGHTONA_JWT_TOKEN", "")
		t.Setenv("NIGHTONA_ORGANIZATION_ID", "")
...
func TestStreamBuildLogsToChannel(t *testing.T) {
	t.Run("streams logs successfully", func(t *testing.T) {
		...
		t.Setenv("NIGHTONA_API_KEY", "test-api-key")
		t.Setenv("NIGHTONA_API_URL", server.URL)
		t.Setenv("NIGHTONA_JWT_TOKEN", "")
		t.Setenv("NIGHTONA_ORGANIZATION_ID", "")
...
func TestServerURLConstruction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("NIGHTONA_API_KEY", "test-api-key")
			t.Setenv("NIGHTONA_API_URL", tt.apiURL)
			t.Setenv("NIGHTONA_JWT_TOKEN", "")
			t.Setenv("NIGHTONA_ORGANIZATION_ID", "")
...
func TestJWTAuthentication(t *testing.T) {
	t.Setenv("NIGHTONA_API_KEY", "")
	t.Setenv("NIGHTONA_API_URL", "")
	t.Setenv("NIGHTONA_JWT_TOKEN", "test-jwt-token")
	t.Setenv("NIGHTONA_ORGANIZATION_ID", "test-org-id")
	...
	cfg := client.apiClient.GetConfig()
	orgHeader, exists := cfg.DefaultHeader["X-Nightona-Organization-ID"]
...
func TestSDKSourceHeader(t *testing.T) {
	...
	cfg := client.apiClient.GetConfig()
	sourceHeader, exists := cfg.DefaultHeader["X-Nightona-Source"]
	assert.True(t, exists)
...
func TestNewClientServerURLFallback(t *testing.T) {
	...
	t.Setenv("NIGHTONA_JWT_TOKEN", "")
	t.Setenv("NIGHTONA_ORGANIZATION_ID", "")
	t.Setenv("NIGHTONA_SERVER_URL", "https://server-url.example/api")
...
func TestHandleAPIErrorUsesOpenAPIBody(t *testing.T) {
	...
	assert.Equal(t, "*errors.NightonaRateLimitError", fmt.Sprintf("%T", converted))
}
