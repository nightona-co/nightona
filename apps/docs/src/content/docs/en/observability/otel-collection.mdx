...
description: Enable distributed tracing for Nightona SDK operations using OpenTelemetry.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

OpenTelemetry (OTEL) tracing allows you to monitor and debug your Nightona SDK operations by collecting distributed traces. This is particularly useful for understanding performance bottlenecks, debugging issues, and gaining visibility into your sandbox operations.

---

## Sandbox Telemetry Collection

Nightona can collect traces, logs, and metrics directly from your sandboxes. This provides complete observability across your entire Nightona environment, from [SDK calls](#sdk-tracing-configuration) to sandbox runtime behavior.
...
- `nightona.sandbox.cpu.utilization` - CPU usage percentage (0-100%)
- `nightona.sandbox.cpu.limit` - CPU cores limit
- `nightona.sandbox.memory.utilization` - Memory usage percentage (0-100%)
- `nightona.sandbox.memory.usage` - Memory used in bytes
- `nightona.sandbox.memory.limit` - Memory limit in bytes
- `nightona.sandbox.filesystem.utilization` - Disk usage percentage (0-100%)
- `nightona.sandbox.filesystem.usage` - Disk space used in bytes
- `nightona.sandbox.filesystem.available` - Disk space available in bytes
- `nightona.sandbox.filesystem.total` - Total disk space in bytes
...
Logs, traces, and metrics collected from sandboxes can be viewed directly in the Nightona Dashboard. Open the **Sandbox Details** sheet for any sandbox and use the **Logs**, **Traces**, and **Metrics** tabs to inspect the collected telemetry data.

:::note
Nightona retains sandbox telemetry data for **3 days**. If you need to keep the data for longer, it is recommended that you connect your own OTLP-compatible collector using the [sandbox collection configuration](#configure-sandbox-collection).
:::
...
- **SDK tracing only**: Monitor Nightona API operations and SDK calls
...
1. Navigate to the [Nightona Dashboard](https://app.daytona.io)
...
- `nightona_organization_id` - The organization the sandbox belongs to
- `nightona_region_id` - The region the sandbox is running in
- `nightona_snapshot` - The snapshot used to create the sandbox

#### Custom Resource Labels

You can attach additional resource labels by setting the `NIGHTONA_SANDBOX_OTEL_EXTRA_LABELS` environment variable on a sandbox. Labels are specified as a comma-separated list of `key=value` pairs:

```bash
NIGHTONA_SANDBOX_OTEL_EXTRA_LABELS="team=backend,env=staging,app=my-service"
```
...
In addition to per-sandbox telemetry, Nightona exports organization-level resource metrics to your configured OTLP endpoint. These metrics are pushed every 60 seconds and provide a high-level view of resource consumption and quotas across your organization.

### Exported Metrics

| Metric                          | Unit      | Description                                         |
| ------------------------------- | --------- | --------------------------------------------------- |
| `nightona.sandbox.used_cpu`      | cpu cores | Total CPU currently consumed by active sandboxes    |
| `nightona.sandbox.used_ram`      | GiB       | Total memory currently consumed by active sandboxes |
| `nightona.sandbox.used_storage`  | GiB       | Total disk currently consumed by sandboxes          |
| `nightona.sandbox.total_cpu`     | cpu cores | Total CPU quota for the organization                |
| `nightona.sandbox.total_ram`     | GiB       | Total memory quota for the organization             |
| `nightona.sandbox.total_storage` | GiB       | Total disk quota for the organization               |
...
When enabled, the Nightona SDK automatically instruments all SDK operations including:

- Sandbox creation, starting, stopping, and deletion
- File system operations
- Code execution
- Process management
- HTTP requests to the Nightona API
...
To enable OpenTelemetry tracing, pass the `otelEnabled` flag when initializing the Nightona client:

Alternatively, you can set the `NIGHTONA_OTEL_ENABLED` environment variable to `true` instead of passing the configuration option:

```bash
export NIGHTONA_OTEL_ENABLED=true
```
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    async with Nightona(NightonaConfig(otel_enabled=True)) as nightona:
    ...
    ```python
    nightona = Nightona(NightonaConfig(otel_enabled=True))
    try:
        sandbox = await nightona.create()
        # All operations will be traced
    finally:
        await nightona.close()  # Flushes traces
    ```
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    await using nightona = new Nightona({ otelEnabled: true })
    ...
    ```typescript
    const nightona = new Nightona({ otelEnabled: true })

    try {
      const sandbox = await nightona.create()
      // All operations will be traced
    } finally {
      await nightona[Symbol.asyncDispose]()  // Flushes traces
    }
    ```
  ...
  <TabItem label="Ruby" icon="seti:ruby">
    ...
    config = Nightona::Config.new(otel_enabled: true)
    nightona = Nightona::Nightona.new(config)

    sandbox = nightona.create
    # All operations will be traced

    nightona.close # Flushes traces
    ```

    Or with `ensure` block:
    ```ruby
    nightona = Nightona::Nightona.new(
      Nightona::Config.new(otel_enabled: true)
    )
    begin
      sandbox = nightona.create
      # All operations will be traced
...
:::note
The legacy `_experimental.otelEnabled` option (and the `NIGHTONA_EXPERIMENTAL_OTEL_ENABLED` env var) still work as deprecated aliases for backwards compatibility.
:::
...
Use the base endpoint without a `/v1/...` path — the path is appended automatically. Generate an API key under **Datadog → Organization Settings → API Keys**. See the [Datadog dashboard example](https://github.com/nightona-co/nightona/tree/main/examples/otel-dashboards/datadog) for an importable dashboard and verification steps.

Once data is flowing, your metrics appear under **Metrics → Summary** (search for `nightona.sandbox`), where you can also confirm the exact tag keys (e.g. `service`, `region.id`) Datadog assigned.
...
Here's a complete example showing how to use OpenTelemetry tracing with the Nightona SDK:
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    async def main():
        # Initialize Nightona with OTEL enabled
        async with Nightona(NightonaConfig(otel_enabled=True)) as nightona:
...
print(f"NumPy version: {np.__version__}")
      ...
      // Traces are automatically flushed when the nightona instance is disposed
    }
...
The Nightona SDK automatically instruments the following operations:
...
- All API calls to the Nightona backend
...
- [New Relic](https://github.com/nightona-co/nightona/tree/main/examples/otel-dashboards/new-relic)
- [Grafana](https://github.com/nightona-co/nightona/tree/main/examples/otel-dashboards/grafana)
- [Datadog](https://github.com/nightona-co/nightona/tree/main/examples/otel-dashboards/datadog)
...
- Check that you're properly closing/disposing the Nightona instance to flush traces
...
1. **Always close the client**: Use `async with` (Python), `await using` (TypeScript), `defer client.Close()` (Go), or `ensure nightona.close` (Ruby) to ensure traces are properly flushed
