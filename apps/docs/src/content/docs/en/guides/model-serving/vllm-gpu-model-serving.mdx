...
description: Spin up an OpenAI-compatible vLLM endpoint on a Nightona GPU sandbox and query it from anywhere.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide demonstrates how to serve an open-weights model on a Nightona [GPU sandbox](/docs/en/sandboxes#gpu-sandboxes) with [vLLM](https://docs.vllm.ai/en/stable/) and query it from anywhere through a token-authenticated preview URL.
...
cd nightona/guides/python/model-serving/vllm
```
...
This installs the `nightona` SDK along with the `openai` and `litellm` clients used by the query examples.

#### Configure Environment

Get your Nightona API key from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys) and set it in a `.env` file:
...
The preview link is the piece that exposes the server outside the sandbox: `pv.url` is reachable from anywhere, and requests authenticate with the `x-nightona-preview-token` header. The URL follows the structure `https://{port}-{sandboxId}.{nightonaProxyDomain}`, as described in the [preview docs](/docs/en/preview). The same URL and token the script uses for health checks are the ones your clients will use for inference.
...
export ENDPOINT=https://8000-{sandboxId}.{nightonaProxyDomain}
export TOKEN={previewToken}

sandbox left UP: {sandboxId}
  reconnect:  nightona.get('{sandboxId}')
  delete:     nightona.get('{sandboxId}').delete()
```
...
<Tabs>
  ...
  <TabItem label="LiteLLM" icon="seti:python">
    ...
    resp = litellm.completion(
        ...
        api_key="EMPTY",
        extra_headers={"x-nightona-preview-token": os.environ["TOKEN"]},
...
The only Nightona-specific detail in any of these is the `x-nightona-preview-token` header. Everything else is the standard OpenAI API surface. The next three examples continue with the OpenAI SDK, following `query_openai.py` through streaming, reasoning, and tool calling.
...
Two independent layers decide who can reach the model: Nightona's preview proxy in front of the sandbox, and vLLM's own API key check inside it. The guide has used one mode of the proxy layer so far; here is the full picture.

#### The Nightona Layer: Preview Links
...
The two layers do not guard the same surface, though. The vLLM key covers the inference routes (`/v1` and similar prefixes), while [other endpoints on the same server](https://docs.vllm.ai/en/stable/usage/security/) accept requests without it. The Nightona token gates everything on the port.

That makes **public preview plus vLLM API key** a combination for sharing with people you broadly trust: the endpoint behaves like a standard OpenAI-style API, configured with nothing but a base URL and an `api_key`, so it works in any tool that accepts only those two fields. For anything more exposed than that, the Nightona layer is the better fit.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    sb = nightona.get("sandbox-id")  # printed by serve_vllm.py
