---
title: Build a Generative-UI Coding Agent with CopilotKit and Nightona
description: Wire a CopilotKit Built-in Agent to a Nightona sandbox with full shell and filesystem access, where every tool call streams into the chat as generative UI and any hosted process embeds as a live iframe.
---
...
This guide demonstrates how to build a [CopilotKit](https://docs.showcase.copilotkit.ai/) Built-in Agent backed by a [Nightona](https://www.daytona.io/) sandbox with full shell and filesystem access. The agent handles whatever a developer might do at a terminal: build apps, debug or analyze code, run scripts, work with data, install packages.
...
Clone the [Nightona repository](https://github.com/nightona-co/nightona) and navigate to the example directory:
...
- **Nightona API key**, from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys)
...
The agent exposes 11 tools, defined with `defineTool` from `@copilotkit/runtime/v2`. Each is a thin wrapper around a [Nightona TypeScript SDK](https://www.daytona.io/docs/en/typescript-sdk/) call:

| Tool | Nightona SDK call |
|---|---|
| `createSandbox` (with optional `envVars`, `labels`, `autoStopInterval`) | `nightona.create({ public: true, envVars, labels, autoStopInterval })` |
...
The backend pulls the runtime helpers from `@copilotkit/runtime`, the v2 agent + tool helpers from `@copilotkit/runtime/v2`, the Nightona client, the Next.js request type, and Zod for tool parameter schemas:
...
const nightona = new Nightona({ apiKey: process.env.NIGHTONA_API_KEY })
```
...
```text
You are a coding agent with shell access to a fresh Nightona sandbox.
...
2. ALWAYS bind the dev server to 0.0.0.0 or the Nightona proxy will not reach
   it. Cheat sheet:
...
const createSandbox = defineTool({
  name: 'createSandbox',
  description:
    'Create a fresh Nightona sandbox with public preview URLs enabled. Call ONCE at session start; reuse the returned sandboxId for every subsequent tool call. Optionally inject environment variables, labels, or change the auto-stop interval.',
  parameters: z.object({
    envVars: z
      .record(z.string())
      .optional()
...
`getPreviewUrl` returns the Nightona proxy URL for any port the sandbox has open. It is the standalone counterpart to `startWebServer`: use it when the agent has already brought up a hosted process by other means (a previous `runCommand`, an already-running service) and just needs the URL to surface as an iframe, without spawning a new dev server:
...
The remaining six tools (`readFile`, `listFiles`, `findFiles`, `searchFiles`, `replaceInFiles`, `getFileDetails`) follow the same shape: a Zod schema for inputs, a one-call wrapper around the Nightona SDK in `execute`, and a structured object returned to the frontend.
...
●  Live preview
<iframe src="https://5173-a0ffcc3d-7753-4d52-89d0-6c595c60626a.nightonaproxy01.net" /> — live preview of the running Snake game

Done — the Snake game is running here: https://5173-a0ffcc3d-7753-4d52-89d0-6c595c60626a.nightonaproxy01.net
```

<Image
  src={copilotkitSnakeGame}
  alt="Snake game built and previewed live in the chat by the CopilotKit + Nightona coding agent"
  width={600}
...
- **All code execution happens inside an isolated Nightona sandbox**, never on the host running the chat.
