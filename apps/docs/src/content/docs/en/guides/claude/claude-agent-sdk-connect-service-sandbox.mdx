---
title: Build a Two-Agent Coding System with Claude and Nightona
description: Step-by-step guide to building a dual-agent coding system using Claude Agent SDK and Nightona sandboxes.
---

This guide demonstrates how to run a two-agent autonomous coding system using the [Claude Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview) and Nightona sandboxes. The system consists of a **Project Manager Agent** (local) and a **Developer Agent** (in-sandbox), enabling advanced delegation, planning, and secure code execution.

The Project Manager Agent runs locally and uses the basic Anthropic interface with the `claude-sonnet-4-20250514` model for high-level planning and task delegation. The Developer Agent runs inside the Nightona sandbox and is created using the Claude Agent SDK, which leverages Claude Code for advanced coding and automation capabilities. This architecture separates high-level planning from low-level code execution for more robust automation.
...
When the main module is launched, a Nightona sandbox is created for the Developer Agent, and a Project Manager Agent is initialized locally. Interaction with the system occurs via a command line chat interface. The Project Manager Agent receives prompts, plans the workflow, and delegates coding tasks to the Developer Agent. The Developer Agent executes tasks in the sandbox and streams results back to the Project Manager, who reviews and coordinates further actions. All logs and outputs from both agents are streamed in real time to the terminal, providing full visibility into the process as it is managed by the Project Manager Agent.

The Developer Agent can also host web apps and provide preview links using [Nightona Preview Links](https://www.daytona.io/docs/en/preview/). The Project Manager Agent will present these links and summarize the results for you.
...
cd nightona/guides/typescript/anthropic/multi-agent-claude-sdk
```
...
The `SANDBOX_ANTHROPIC_API_KEY` is passed into the Nightona sandbox environment and is accessible to any code executed inside the sandbox.
:::
...
2. **Awareness of Developer Agent:**
  - The Project Manager Agent knows that a Developer Agent is available inside a Nightona sandbox and can be invoked as needed.
3. **Task Delegation:**
...
1. **Provisioning:**
  - The Developer Agent is provisioned inside a Nightona sandbox and is responsible for executing coding tasks.
2. **SDK Installation:**
...
When you exit the main program, the Nightona sandbox and all files are automatically deleted.
