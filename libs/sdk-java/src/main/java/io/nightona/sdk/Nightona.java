...
import io.nightona.api.client.api.SandboxApi;
import io.nightona.api.client.model.CreateBuildInfo;
import io.nightona.api.client.model.CreateSandbox;
import io.nightona.api.client.model.SandboxVolume;
 ...
 * @see Sandbox
 */
public class Nightona implements AutoCloseable {
    public static final String CODE_TOOLBOX_LANGUAGE_LABEL = "code-toolbox-language";

    private final NightonaConfig config;
    private final io.nightona.api.client.ApiClient apiClient;
     ...
     * @throws NightonaException if configuration is invalid or missing credentials
     */
    public Nightona(NightonaConfig config) {
        if (config == null || config.getApiKey() == null || config.getApiKey().isEmpty()) {
            throw new NightonaException(
                    "Authentication required: set NIGHTONA_API_KEY environment variable or pass apiKey in NightonaConfig");
        }
        this.config = config;
        this.apiClient = createMainApiClient(config);
        this.sandboxApi = new SandboxApi(apiClient);
        this.snapshot = new SnapshotService(new io.nightona.api.client.api.SnapshotsApi(apiClient), apiClient.getHttpClient(), config.getApiKey());
        this.volume = new VolumeService(new io.nightona.api.client.api.VolumesApi(apiClient));
    }
     ...
     * @throws NightonaException if creation or startup fails
     */
     ...
     * @return created and started {@link Sandbox}
     * @throws NightonaException if creation fails or the Sandbox does not start in time
     */
    public Sandbox create(CreateSandboxFromSnapshotParams params, long timeoutSeconds) {
        ...
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.createSandbox(body, null));
     ...
     * @throws NightonaException if the Sandbox is not found or request fails
     */
    public Sandbox get(String sandboxIdOrName) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(sandboxIdOrName, null, null));
     ...
     * {@code Nightona.list}" on {@link Sandbox} (env, networkBlockAll, networkAllowList, volumes,
     ...
     *
     * <pre>{@code
     * try (Stream<Sandbox> stream = nightona.listStream(query)) {
    ...
    PageResult fetchSandboxPage(ListSandboxesQuery query, String cursor) {
        ...
        String id = null;
        String name = null;
        List<io.nightona.api.client.model.SandboxState> states = null;
        ...
        io.nightona.api.client.model.SandboxListSortField sort = null;
        io.nightona.api.client.model.SandboxListSortDirection order = null;
        ...
        if (query != null) {
            ...
            if (query.getStates() != null) {
                ...
                for (io.nightona.sdk.model.SandboxState s : query.getStates()) {
        ...
        final List<io.nightona.api.client.model.SandboxState> fStates = states;
        ...
        final io.nightona.api.client.model.SandboxListSortField fSort = sort;
        final io.nightona.api.client.model.SandboxListSortDirection fOrder = order;

        io.nightona.api.client.model.ListSandboxesResponse result = ExceptionMapper.callMain(() -> sandboxApi.listSandboxes(
                null,
                fCursor,
                fLimitVal,
                fId,
        ...
        if (result != null && result.getItems() != null) {
            for (io.nightona.api.client.model.SandboxListItem item : result.getItems()) {
    ...
    /**
     * Cursor-based iterator that lazily pulls pages from the Nightona API.
     *
    ...
    private static final class SandboxIterator implements Iterator<Sandbox> {
        private final Nightona nightona;
        ...
        SandboxIterator(Nightona nightona, ListSandboxesQuery query) {
            this.nightona = nightona;
            this.query = query;
        }
        ...
        private void advanceIfNeeded() {
            while ((page == null || pageIndex >= page.size()) && !exhausted) {
                ...
                PageResult result = nightona.fetchSandboxPage(query, cursor);
    ...
    private CreateSandbox baseSandboxBody(io.nightona.sdk.model.CreateSandboxParams params) {
        ...
        if (params.getVolumes() != null) {
            ...
            for (io.nightona.sdk.model.VolumeMount mount : params.getVolumes()) {
    ...
    private io.nightona.api.client.ApiClient createMainApiClient(NightonaConfig cfg) {
        io.nightona.api.client.ApiClient client = new io.nightona.api.client.ApiClient();
        client.setBasePath(trimTrailingSlash(cfg.getApiUrl()));
        client.setBearerToken(cfg.getApiKey());
        String sdkVersion = Nightona.class.getPackage().getImplementationVersion();
        if (sdkVersion == null) sdkVersion = "dev";
        client.addDefaultHeader("X-Nightona-Source", "sdk-java");
        client.addDefaultHeader("X-Nightona-SDK-Version", sdkVersion);
    ...
    private void ensureOauth2AuthEntry(io.nightona.api.client.ApiClient client) {
        if (client.getAuthentications().containsKey("oauth2")) {
            return;
        }
        try {
            Field authField = io.nightona.api.client.ApiClient.class.getDeclaredField("authentications");
            authField.setAccessible(true);
            Map<String, io.nightona.api.client.auth.Authentication> auths =
                    new HashMap<String, io.nightona.api.client.auth.Authentication>(
                            (Map<String, io.nightona.api.client.auth.Authentication>) authField.get(client)
                    );
            auths.put("oauth2", (q, h, c, p, m, u) -> {
            });
            authField.set(client, Collections.unmodifiableMap(auths));
        } catch (ReflectiveOperationException e) {
            throw new NightonaException("Failed to initialize authentication", e);
        }
    }
    ...
    private String toJson(Object value) {
        try {
            ...
            throw new NightonaException("Failed to serialize JSON", e);
        }
    }
        ...
        while (true) {
            if (timeoutSeconds > 0) {
                ...
                if (elapsed > timeoutSeconds) {
                    throw new NightonaException("Sandbox build pending for more than " + timeoutSeconds + " seconds");
                }
            }
            io.nightona.api.client.model.Sandbox s = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(sandboxId, null, null));
    ...
    private void streamSandboxBuildLogs(String sandboxId, java.util.function.Consumer<String> onLog, long timeoutSeconds, long startTime) {
        io.nightona.api.client.model.Url logsUrl = ExceptionMapper.callMain(() -> sandboxApi.getBuildLogsUrl(sandboxId, null));
