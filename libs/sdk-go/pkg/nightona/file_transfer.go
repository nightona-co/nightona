...
func streamDownloadFile(cfg *toolbox.Configuration, remotePath string, ctx context.Context, onProgress func(DownloadProgress)) (io.ReadCloser, error) {
	if len(cfg.Servers) == 0 {
		return nil, errors.NewNightonaError("Toolbox client is not configured", 0, nil)
	}

	requestBody, err := json.Marshal(toolbox.NewFilesDownloadRequest([]string{remotePath}))
	if err != nil {
		return nil, errors.NewNightonaError(fmt.Sprintf("Failed to encode download request: %v", err), 0, nil)
	}

	endpoint := strings.TrimRight(cfg.Servers[0].URL, "/") + "/files/bulk-download"
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(requestBody))
	if err != nil {
		return nil, errors.NewNightonaError(fmt.Sprintf("Failed to create download request: %v", err), 0, nil)
	}
	...
	if err != nil {
		return nil, errors.NewNightonaError(fmt.Sprintf("Failed to download file stream: %v", err), 0, nil)
	}
	...
	if resp.StatusCode >= http.StatusMultipleChoices {
		...
		if readErr != nil {
			return nil, errors.NewNightonaError(fmt.Sprintf("Failed to read download error response: %v", readErr), resp.StatusCode, resp.Header)
		}

		return nil, errors.NewNightonaErrorFromBody(body, resp.StatusCode, resp.Header)
	}

	_, params, err := mime.ParseMediaType(resp.Header.Get("Content-Type"))
	if err != nil {
		_ = resp.Body.Close()
		return nil, errors.NewNightonaError(fmt.Sprintf("Failed to parse multipart response: %v", err), resp.StatusCode, resp.Header)
	}

	boundary := params["boundary"]
	if boundary == "" {
		_ = resp.Body.Close()
		return nil, errors.NewNightonaError("Missing multipart boundary in download response", resp.StatusCode, resp.Header)
	}
	...
	for {
		...
		if err == io.EOF {
			_ = resp.Body.Close()
			return nil, errors.NewNightonaError("File stream not found in download response", resp.StatusCode, resp.Header)
		}
		if err != nil {
			_ = resp.Body.Close()
			return nil, errors.NewNightonaError(fmt.Sprintf("Failed to read multipart download response: %v", err), resp.StatusCode, resp.Header)
		}
		...
		case "error":
			...
			_ = resp.Body.Close()
			if readErr != nil {
				return nil, errors.NewNightonaError(fmt.Sprintf("Failed to read download error part: %v", readErr), resp.StatusCode, resp.Header)
			}
			return nil, errors.NewNightonaErrorFromBody(body, resp.StatusCode, resp.Header)
		}
	}
...
func streamUploadFile(cfg *toolbox.Configuration, remotePath string, source io.Reader, opts *uploadStreamConfig, ctx context.Context) error {
	if len(cfg.Servers) == 0 {
		return errors.NewNightonaError("Toolbox client is not configured", 0, nil)
	}
	...
	if err != nil {
		...
		<-errCh
		return errors.NewNightonaError(fmt.Sprintf("Failed to create upload request: %v", err), 0, nil)
	}
	for key, value := range cfg.DefaultHeader {
		req.Header.Set(key, value)
	}
	...
	if err != nil {
		_ = pw.CloseWithError(err)
		<-errCh
		return errors.NewNightonaError(fmt.Sprintf("Failed to upload file: %v", err), 0, nil)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(resp.Body)
		returnErr := errors.NewNightonaErrorFromBody(body, resp.StatusCode, resp.Header)
