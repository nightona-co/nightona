---
title: Build a Multi-Language Benchmark Agent with the Vercel AI SDK and Nightona
description: Use the Vercel AI SDK's ToolLoopAgent with five sandbox tools to build a benchmarking agent that implements algorithms in Python and TypeScript, times them, plots the comparison with matplotlib, and downloads both the chart and a markdown findings report to your local filesystem - all running inside a Nightona sandbox.
---
...
This guide walks through building a multi-language benchmarking agent with the [Vercel AI SDK](https://ai-sdk.dev/) and [Nightona](/docs/en/sandboxes) sandboxes. The agent has five tools (`runCode`, `runCommand`, `writeFile`, `readFile`, `downloadFile`) and a concrete benchmarking task: implement the Sieve of Eratosthenes in both Python and TypeScript, time them across input sizes, render a matplotlib comparison chart, and deliver two tangible artifacts to your local disk: the chart as a PNG and the findings as a markdown report.

This is the shape of a real coding agent (multi-step, multi-language, multi-tool) producing tangible deliverables rather than just text, running entirely inside Nightona's isolated environment.

---

### 1. Workflow Overview

When you run the script, the agent receives a Sieve of Eratosthenes benchmark task, implements it in two languages, plots the result, and pulls the PNG to your local disk. The transcript below is **excerpted from an actual run** using `claude-sonnet-4-6` against a real Nightona sandbox. Source-code arguments inside `runCode` and `writeFile` calls are abridged (marked by `...`); tool-call shapes, results, file sizes, and the final answer are verbatim:
...
cd nightona/guides/typescript/vercel-ai-sdk/multi-language-benchmark-agent
```
...
- **Nightona API key:** [Nightona Dashboard](https://app.daytona.io/dashboard/keys)
...
The example consists of a single Node.js script (`src/index.ts`) that runs on your local machine. It uses the Nightona SDK to manage a sandbox and the Vercel AI SDK's `ToolLoopAgent` to drive the multi-step tool-calling loop.

#### Defining the Five Tools

Each tool wraps a single Nightona primitive and has a single, non-overlapping job. The agent never has to choose between two paths for the same goal.
...
When the script exits (whether normally or on error), the Nightona sandbox is deleted in a `finally` block. The downloaded `sieve_benchmark.png` and `findings.md` remain on your local disk. Those are the artifacts the agent produced for you to inspect after the run.
