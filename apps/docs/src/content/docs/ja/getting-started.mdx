...
Nightona SDKはNightonaと連携するための公式[Python](/docs/ja/python-sdk)および[TypeScript](/docs/ja/typescript-sdk)インターフェースを提供し、開発環境をプログラムで管理し、コードを実行できます。[Python SDK](/docs/ja/python-sdk)は同期・非同期の両方のプログラミングモデルをサポートしており、非同期クラスには`Async`の接頭辞が付きます。

手順に従って、AIエージェント向けの最初のNightona サンドボックス（Nightonaが管理する隔離された一時的な実行環境）を作成・実行しましょう。

環境変数の設定やステージング環境での実験的機能へのアクセスなど、追加の設定手順については[Configuration](/docs/ja/configuration)を参照してください。

## Nightona SDK をインストールする

Nightona は、Nightona プラットフォームと連携するための公式 Python および TypeScript の SDK を提供しています。お好みの方法で SDK をインストールしてください。
...
次のコードを実行して、Nightona のサンドボックスを作成し、コマンドを実行します。
...
次のスニペットは、シンプルな Flask アプリを含むファイルを Nightona のサンドボックスにアップロードします。Web サーバーはポート `3000` で起動し、提供されたプレビューURLからアクセスできます。

<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ```python
    from nightona import Nightona, NightonaConfig, SessionExecuteRequest
    ...
    def hello():
        ...
        <body style="display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background-color: #0a0a0a; font-family: Arial, sans-serif;">
            <div style="text-align: center; padding: 2rem; border-radius: 10px; background-color: white; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                <img src="https://raw.githubusercontent.com/Amartuvshins0404/nightona/main/assets/images/Nightona-logotype-black.png" alt="Nightona Logo" style="width: 180px; margin: 10px 0px;">
                <p>This web app is running in a Nightona sandbox!</p>
            </div>
        </body>
        </html>
        """
...
次のスニペットは、Anthropic API を用いて LLM に接続し、Claude に 25 の階乗を求めるコードの生成を依頼し、そのコードを Nightona のサンドボックス（Nightonaが管理する隔離された一時的な実行環境）内で実行します。
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    nightona = Nightona(NightonaConfig())
    ...
    NIGHTONA_API_KEY="your-nightona-api-key"
    NIGHTONA_TARGET=us
...
Nightona SDK の [Python の例](https://github.com/Amartuvshins0404/nightona/tree/main/examples/python) または [TypeScript/JavaScript の例](https://github.com/Amartuvshins0404/nightona/tree/main/examples/typescript) を使ってサンドボックス（Nightonaが管理する隔離された一時的な実行環境）を作成し、コードを実行しましょう。

LLM を活用して Nightona 上での開発を加速できます。/llms.txt ファイルをコピーし、プロジェクトやチャットのコンテキストに含めてください: [llms-full.txt](https://www.daytona.io/docs/llms-full.txt) または [llms.txt](https://www.daytona.io/docs/llms.txt)

[GitHub](https://github.com/Amartuvshins0404/nightona) 上の Nightona SDK リポジトリを参照して、さらに詳しく学びましょう。

## ViteプロジェクトでのNightona

ViteベースのプロジェクトでNightona SDKを使用する場合、互換性を確保するためにNodeのポリフィルを設定する必要があります。`vite.config.ts` のplugins配列に以下の設定を追加してください：
...
## Next.js プロジェクトにおける Nightona

Next.js プロジェクトで Nightona SDK を使用する場合、利用中のバンドラーに応じて Webpack や Turbopack との互換性を保つため、Node のポリフィルを設定する必要があります。次の設定を `next.config.ts` に追加してください:
...
## Nightona CLI のセットアップ

[ローカルデバイス上のイメージ](/docs/ja/snapshots#using-a-local-image)を使用する場合、またはコマンドラインインターフェースでサンドボックスを管理したい場合は、次のコマンドで Nightona CLI をインストールしてください:
