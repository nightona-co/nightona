...
description: Mount external object storage (Amazon S3, Cloudflare R2, Google Cloud Storage, Azure Blob) and filesystems into Nightona sandboxes using FUSE.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

Mount object storage (Amazon S3, Cloudflare R2, Google Cloud Storage, Azure Blob) and filesystems like Archil and MesaFS into a Nightona sandbox as a regular directory. The sandbox reads from and writes to the bucket as if it were a local directory, so existing tools, scripts, and agents work without changes. This is useful for bringing in datasets, model weights, or build artifacts that already live in your own cloud account.

External storage mounts and [Nightona Volumes](/docs/en/volumes/) are complementary FUSE-based mechanisms — both expose remote object storage as a regular sandbox directory, both can be shared across sandboxes, and both persist beyond any individual sandbox's lifetime. The main distinction is **where the data physically lives**: Nightona Volumes are hosted on Nightona's own S3-compatible object store, while external mounts connect to a bucket or filesystem hosted on another provider (Amazon S3, Cloudflare R2, GCS, Azure Blob, Archil, MesaFS).

External storage is mounted using FUSE. Nightona supports two approaches, and each provider section below shows both — pick whichever fits your workflow:
...
```python
from nightona import CreateSnapshotParams, Nightona, Image
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-s3', image: image),
...
image := nightona.Base("daytonaio/sandbox").
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-s3", image, System.out::println);
        }
    }
}
```
...
sandbox.process.exec(f"echo 'hello from Nightona' > {mount_path}/hello.txt")
...
// Read and write through the mount as if it were a local directory
await sandbox.process.executeCommand(`echo 'hello from Nightona' > ${mountPath}/hello.txt`)
...
# Read and write through the mount as if it were a local directory
sandbox.process.exec(command: "echo 'hello from Nightona' > #{mount_path}/hello.txt")
...
// Read and write through the mount as if it were a local directory
if _, err := sandbox.Process.ExecuteCommand(ctx, "echo 'hello from Nightona' > "+mountPath+"/hello.txt"); err != nil {
	log.Fatal(err)
}
    ...
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            sandbox.getProcess().executeCommand(
                "echo 'hello from Nightona' > " + mountPath + "/hello.txt");
...
```python
import os
from nightona import CreateSandboxBaseParams, Nightona
...
sandbox = nightona.create(
  Nightona::CreateSandboxBaseParams.new(
    env_vars: {
      'AWS_ACCESS_KEY_ID' => ENV.fetch('AWS_ACCESS_KEY_ID'),
      'AWS_SECRET_ACCESS_KEY' => ENV.fetch('AWS_SECRET_ACCESS_KEY')
    }
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-r2', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-r2", image, System.out::println);
        }
    }
}
```
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-tigris', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-tigris", image, System.out::println);
        }
    }
}
```
...
import io.nightona.sdk.Nightona;
import io.nightona.sdk.Sandbox;
import io.nightona.sdk.model.CreateSandboxBaseParams;
import io.nightona.sdk.model.ExecuteResponse;
...
This pattern fits Nightona sandboxes when each sandbox needs a writable copy of a shared dataset (model weights, fixtures, golden data) without duplicating it on every launch.
...
:::note
Nightona's base image is Debian Trixie, but Google has not published a Trixie-specific gcsfuse repository yet. The `gcsfuse-bookworm` repo packages run cleanly on Trixie, so we use them in the snippets below.
:::
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-gcs', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-gcs", image, System.out::println);
        }
    }
}
```
...
sandbox = nightona.create(CreateSandboxFromSnapshotParams(snapshot="fuse-gcs"))
...
const sandbox = await nightona.create({ snapshot: 'fuse-gcs' })
...
sandbox = nightona.create(
  Nightona::CreateSandboxFromSnapshotParams.new(snapshot: 'fuse-gcs')
)
...
sandbox = nightona.create(CreateSandboxBaseParams())
...
sandbox = nightona.create(Nightona::CreateSandboxBaseParams.new)
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            Sandbox sandbox = nightona.create(new CreateSandboxFromSnapshotParams());
...
:::caution
Nightona's base image is Debian Trixie. Microsoft's `blobfuse2` Bookworm binary links against `libfuse3.so.3`, but Trixie's `fuse3` package bumped the SONAME to `.4`, so `libfuse3.so.3` is missing on disk. The snapshot recipe below creates a compatibility symlink. Without it, `blobfuse2` fails with `libfuse3.so.3: cannot open shared object file`.
:::
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-azure', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-azure", image, System.out::println);
        }
    }
}
```
...
sandbox = nightona.create(CreateSandboxFromSnapshotParams(snapshot="fuse-azure"))
...
const sandbox = await nightona.create({ snapshot: 'fuse-azure' })
...
sandbox = nightona.create(
  Nightona::CreateSandboxFromSnapshotParams.new(snapshot: 'fuse-azure')
)
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-archil', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-archil", image, System.out::println);
        }
    }
}
```
...
Pass `ARCHIL_MOUNT_TOKEN`, `ARCHIL_REGION`, and `ARCHIL_DISK` to the sandbox via `envVars`. Your code then mounts the disk at `/home/daytona/archil` and hands ownership to the `nightona` user so non-root processes can read and write through the mount.
...
sandbox.process.exec(f"sudo chown nightona:nightona {mount_path}")
...
await sandbox.process.executeCommand(`sudo chown nightona:nightona ${mountPath}`)
...
sandbox.process.exec(command: "sudo chown nightona:nightona #{mount_path}")
...
if _, err := sandbox.Process.ExecuteCommand(ctx,
	"sudo chown nightona:nightona "+mountPath); err != nil {
	log.Fatal(err)
}
    ...
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            sandbox.getProcess().executeCommand(
                "sudo --preserve-env=ARCHIL_MOUNT_TOKEN archil mount "
                + "$ARCHIL_DISK " + mountPath + " --region $ARCHIL_REGION");
            sandbox.getProcess().executeCommand("sudo chown nightona:nightona " + mountPath);
...
[MesaFS ↗](https://mesa.dev) is an agent-native versioned filesystem from Mesa, purpose-built for the same workloads Nightona sandboxes run — parallel agent swarms, shared working memory, structured artifacts, and long-lived state across runs. With MesaFS, instead of mounting a cloud bucket, you mount a Mesa **repository**: a Git-compatible versioned working directory with sub-50ms reads/writes, instant fork/branch operations, and unlimited concurrent writers.
...
nightona.snapshot.create(
  Nightona::CreateSnapshotParams.new(name: 'fuse-mesa', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            nightona.snapshot().create("fuse-mesa", image, System.out::println);
        }
    }
}
```
...
For repo-scoped or path-scoped keys, see Mesa's [auth and permissions guide ↗](https://docs.mesa.dev/content/getting-started/auth-and-permissions). For the full integration recipe, see Mesa's [Nightona guide ↗](https://docs.mesa.dev/content/integration-guides/nightona).

## Unmount

When a sandbox is deleted via `nightona.delete(sandbox)`, the container teardown automatically removes any active FUSE mounts and shuts down their daemons. For normal cleanup, this is all you need — no manual unmount required.
