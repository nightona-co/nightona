...
description: Build LangChain agents that perform secure data analysis using Nightona's isolated sandbox environment.
---
...
This package provides the `NightonaDataAnalysisTool` - LangChain tool integration that enables agents to perform secure Python data analysis in a sandboxed environment. It supports multi-step workflows, file uploads/downloads, and custom result handling, making it ideal for automating data analysis tasks with LangChain agents.
...
You upload your dataset and provide a natural language prompt describing the analysis you want. The agent reasons about your request, determines how to use the `NightonaDataAnalysisTool` to perform the task on your dataset, and executes the analysis securely in a Nightona sandbox.
...
Clone the [repository](https://github.com/nightona-co/nightona) and navigate to the example directory:

```bash
git clone https://github.com/nightona-co/nightona.git
cd nightona/guides/python/langchain/data-analysis/anthropic
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    - `langchain-nightona-data-analysis`: Provides the `NightonaDataAnalysisTool` for LangChain agents
...
Now we'll initialize the `NightonaDataAnalysisTool` and upload our dataset.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    DataAnalysisTool = NightonaDataAnalysisTool(
...
<Tabs>
  ...
  **What happens here:**
    ...
    2. It determines it needs to use the `NightonaDataAnalysisTool`
    3. Agent generates Python code to analyze the data
    4. Code executes securely in the Nightona sandbox
...
**Step 3: Code executes in Nightona sandbox**
...
- **Secure execution:** Code runs in isolated Nightona sandbox
...
The following public methods are available on `NightonaDataAnalysisTool`:
...
This method provides access to the Nightona sandbox instance, allowing you to inspect sandbox properties and metadata, as well as perform any sandbox-related operations. For details on available attributes and methods, see the [Sandbox](#sandbox) data structure section below.
...
:::note
The list of preinstalled packages in a sandbox can be found at [Nightona's Default Snapshot documentation](https://www.daytona.io/docs/en/snapshots/#default-snapshots).
:::
...
#### Sandbox
Represents a Nightona sandbox instance.

See the full structure and API in the [Nightona Python SDK Sandbox documentation](https://www.daytona.io/docs/en/python-sdk/sync/sandbox/#sandbox).