---
title: Train LLMs with Reinforcement Learning Using TRL and Nightona
description: Use parallel Nightona sandboxes to train code-generating LLMs with TRL's GRPO trainer.
---
...
This guide demonstrates how to use Nightona sandboxes to safely execute hundreds of code completions in parallel during reinforcement learning training.

We use [TRL](https://huggingface.co/docs/trl/)'s GRPOTrainer together with 500 Nightona sandboxes evaluating completions concurrently, in order to train the `Qwen3-1.7B-Base` model on some basic code-writing tasks.
...
2. **Evaluate**: Each completion runs in its own Nightona sandbox against a test suite
...
cd nightona/guides/python/reinforcement-learning/trl
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    async def _create_sandbox_pool_async(
        nightona: AsyncNightona, n: int = 10
    ) -> List[AsyncSandbox]:
        print(f"Creating {n} sandboxes...")
        tasks = [nightona.create() for _ in range(n)]
...
TRL's `GRPOTrainer` expects a synchronous reward function, but the Nightona SDK uses async/await for parallel sandbox operations. We bridge these two worlds with a helper:
...
This pattern lets us keep the async parallelism benefits of the Nightona SDK while working within TRL's synchronous training loop. The `run_async` helper blocks until all 500 parallel sandbox evaluations complete, then returns the results.
