...
func (h *PtyHandle) WaitForConnection(ctx context.Context) error {
	...
	for {
		...
		case <-timeoutCtx.Done():
			return errors.NewNightonaTimeoutError("PTY connection timeout")
		case <-ticker.C:
			h.mu.RLock()
			if h.connectionEstablished {
				h.mu.RUnlock()
			...
			if h.err != nil {
				err := h.err
				h.mu.RUnlock()
				return errors.NewNightonaError(*err, 0, nil)
			}
			h.mu.RUnlock()
		}
	}
...
func (h *PtyHandle) SendInput(data []byte) error {
	...
	if !h.connected || h.ws == nil {
		h.mu.RUnlock()
		return errors.NewNightonaError("PTY is not connected", 0, nil)
	}
	ws := h.ws
	h.mu.RUnlock()

	if err := ws.WriteMessage(websocket.BinaryMessage, data); err != nil {
		return errors.NewNightonaError(fmt.Sprintf("Failed to send input to PTY: %v", err), 0, nil)
	}
	...
	for {
		...
		case <-ticker.C:
			...
			if h.err != nil {
				err := *h.err
				h.mu.RUnlock()
				return nil, errors.NewNightonaError(err, 0, nil)
			}
			h.mu.RUnlock()
		}
	}
