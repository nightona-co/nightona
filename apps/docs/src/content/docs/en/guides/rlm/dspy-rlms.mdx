---
title: Run DSPy RLMs on Nightona
description: Use DSPy's RLM module with a Nightona-backed interpreter to safely execute LLM-generated Python code in an isolated cloud sandbox.
---
...
The generated code runs in a REPL, and in this guide we use and present `NightonaInterpreter`, which plugs into DSPy as the code-execution backend so that all generated code runs inside an isolated Nightona cloud sandbox rather than on your machine.
...
cd nightona/guides/python/dspy-rlms
```
...
This installs the DSPy framework and the Nightona SDK. To also run the included demo (which plots results with matplotlib), use
...
```bash
NIGHTONA_API_KEY=your_nightona_api_key
OPENROUTER_API_KEY=your_openrouter_api_key  # or OPENAI_API_KEY / ANTHROPIC_API_KEY
```

:::note
Get your Nightona API key from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys). The LLM provider key depends on which model you configure in your code — the examples use OpenRouter.
:::

### 2. Basic Usage

The following example shows the basic setup — configure a model, create a `NightonaInterpreter`, and pass it to an RLM. The generated code can call `llm_query()` to delegate semantic work back to the LLM:
...
Each RLM call runs an iterative REPL loop. The LLM writes Python code, the code executes in a Nightona sandbox, and the output is fed back to the LLM for the next iteration. Crucially, the generated code can call `llm_query()` to invoke a sub-LLM call — this is how the LLM delegates semantic work (understanding, extraction, classification) to itself while keeping the orchestration logic in Python.
...
3. **Execute** — The code runs inside a Nightona sandbox; any `llm_query()` calls are bridged back to the host LLM
...
```
Host Process                                    Nightona Sandbox
┌──────────────────────────────┐                ┌──────────────────────────────┐
│      NightonaInterpreter      │                │    Broker Server (Flask)     │
│                              │                │                              │
...
- **Isolation** — All generated code runs in a Nightona cloud sandbox, not on your machine
